...
// Inster the main directory for storing original datasets and CPF working files 
	...
	qui reproot, project("cpf_code") roots("here")
	global your_dir "${here}"
...
*/
	...
	* B1A. HILDA
	if strpos("${surveys}", "hilda") do "${Gdr_syntax}/01_HILDA/_au_00_Run_HILDA.do"

	* B1B. KLIPS
	if strpos("${surveys}", "klips") do "${Gdr_syntax}/02_KLIPS/_ko_00_Run_KLIPS.do"

	* B1C. PSID
	if strpos("${surveys}", "psid") do "${Gdr_syntax}/03_PSID/_us_00_Run_PSID.do"

	* B1D. SHP
	if strpos("${surveys}", "shp") do "${Gdr_syntax}/05_SHP/_ch_00_Run_SHP.do"

	* B1E. SOEP
	if strpos("${surveys}", "soep") do "${Gdr_syntax}/06_SOEP/_ge_00_Run_SOEP.do"

	* B1F. UKHLS
	if strpos("${surveys}", "ukhls") do "${Gdr_syntax}/07_UKHLS/_uk_00_Run_UKHLS.do"

	* B1G. LISS
	if strpos("${surveys}", "liss") do "${Gdr_syntax}/08_LISS/_nl_00_Run_LISS.do"
